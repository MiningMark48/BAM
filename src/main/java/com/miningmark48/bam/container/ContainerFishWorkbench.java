...
import com.miningmark48.bam.crafting.FishWorkbenchCraftingManager;
...
public class ContainerFishWorkbench extends Container {
    ...
    public ContainerFishWorkbench(InventoryPlayer player, World world, int x, int y, int z){
        ...
        this.addSlotToContainer(new Slot(craftMatrix, 0, 35, 23));
        this.addSlotToContainer(new Slot(craftMatrix, 1, 35, 46));
        this.addSlotToContainer(new Slot(craftMatrix, 2, 58, 35));

        this.addSlotToContainer(new SlotCrafting(player.player, craftMatrix, craftResult, 3, 124, 35));
    ...
    {
        craftResult.setInventorySlotContents(0, FishWorkbenchCraftingManager.getInstance().findMatchingRecipe(craftMatrix, worldObj));
    }

    @Override
    public boolean canInteractWith(EntityPlayer player){
        if (worldObj.getBlock(posX, posY, posZ) != ModBlocks.fishWorkbench){
            return false;
        }else{
